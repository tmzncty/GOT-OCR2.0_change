# 这个项目是魔改的，相当于根据业务场景走
我只保留了原先的OCR功能，别的都没啥了，不管那么多了。同时使用FP16因为没有安培架构GPU。
# 环境
先把原先项目的跑起来，然后缺什么库装什么库就行，反正我目前没遇到直接安装然后API调不动的情况。
# V100*8并行
## 目的
200W张图片的识别+性能最优
测试可用是，但是这个还不行，临时跑一下没啥。ocr_8gpu.py


# P40*1 API
## 目的
开出来给群友测试玩耍。
也就是ocr_web.py，能用，单卡玩玩没啥事。
